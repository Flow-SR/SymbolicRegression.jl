...
for o in units
    for d in sort(collect(keys(flow_dict[o])))
        ...
        if use_iowork
            ...
            if level=="msoa"
                iowprk += attrtab[geoid2row[o], feat["workpop"]] 
            end
...
save("eng_"*level*"_supp3_X_dist_miorw_odrw.jld2", "X", X)
save("eng_"*level*"_supp3_Y.jld2", "y", y)
save("eng_"*level*"_supp3_sep.jld2", "sep", ori_sep)

=# # Load Existing Data
X = load("eng_"*level*"_supp3_X_dist_odw.jld2", "X")
y = load("eng_"*level*"_supp3_Y.jld2", "y")
ori_sep = load("eng_"*level*"_supp3_sep.jld2", "sep")
